...
class MyWorld(gym.Env):
    ...
    def step1(self, action):
        old_agent_location_ql = np.copy(self.agent_location_ql)

        checker = int(np.sqrt(self.size))
        if action == 0:  # Move right
            if (self.agent_location_ql + 1) % checker != 0 or 0 <= self.agent_location_ql < 9:
                self.agent_location_ql = self.agent_location_ql + 1
            else:
                reward = -1
        elif action == 1:  # Move left
            if (self.agent_location_ql - 1) % checker != 9 or 0 < self.agent_location_ql <= 9:
                self.agent_location_ql = self.agent_location_ql - 1
            else:
                reward = -1
        elif action == 2:  # Move down
            if 0 < self.agent_location_ql + checker < self.size:
                self.agent_location_ql = self.agent_location_ql + checker
            else:
                reward = -1
        elif action == 3:  # Move up
            if 0 <= self.agent_location_ql - checker < self.size:
        ...
        terminated = False
        if self.agent_location_ql in self.obstacles:
            self.agent_location_ql = old_agent_location_ql
            reward = -1
        else:
            if self.agent_location_ql == self.target_location:
                reward = 10
                terminated = True
            else:
                reward = 0

        observation = self.agent_location_ql
        # if self.render_mode == "human" and self.training:
        #     self.render_frame()
    ...
    def render_frame(self):
        ...
        pygame.draw.circle(
            ...
            (0, 128, 0),
            (
                (self.agent_location_ql % int(np.sqrt(self.size)) + 0.5) * pix_square_size,
                (self.agent_location_ql // int(np.sqrt(self.size)) + 0.5) * pix_square_size
            ),
            pix_square_size / 3
        )
...
def learning(train_episodes, max_steps, batch_size, fps, training):
    ...
    lr_actor = 0.0003
    lr_critic = 0.001
    ppo_agent = PPO(state_dim, action_size, lr_actor, lr_critic, gamma, K_epochs, eps_clip)
    if training:
    ...
    if training:
        ...
        for episode in range(train_episodes):
            ...
            for ep in range(1, max_steps + 1):
                ...
                if done:
                    print(
                        f'Episode: {episode:4}/{train_episodes} and step: {ep:4}. Eps: {float(agent.epsilon):.2}, reward {reward}')
                    count += 1
                    break
        ...
        for episode in range(train_episodes):
            ...
            terminated = False
            reward = 0
            epochs, penalties, reward1 = 0, 0, 0
            print('******* EPISODE ', episode, ' *******')

            for ep in range(1, max_steps + 1):
                action = agent.pred(state)
                action1 = ppo_agent.select_action(state1)
                next_state, reward1, terminated, truncated = env.step1(action1)
